...
test.describe('Suite C: Carrinho (INT-03)', () => {
  ...
  test('C2: Carrinho exibe itens adicionados', async ({ page }) => {
    ...
    // Items are rendered as Card (div) components, also accessible via test id
    await expect(page.getByTestId('cart-item-quantity').first()).toBeVisible();
  });
  ...
  test('C3: Alterar quantidade no carrinho', async ({ page }) => {
    ...
    const qtySpan = page.getByTestId('cart-item-quantity').first();
